    ...
    if(glist_isvisible(glist)){
        if(x->x_gui.x_fsf.x_change && x->x_buf[0] && x->x_output_mode){
            ...
            sys_vgui(".x%lx.c itemconfigure %lxnumbox -fill #%06x -text {%s} \n",
                glist_getcanvas(glist), x, x->x_gui.x_fcol, cp);
            ...
            numbox_tilde_ftoa(x);
            sys_vgui(".x%lx.c itemconfigure %lxnumbox -fill #%06x -text {%s} \n",
                glist_getcanvas(glist), x, x->x_gui.x_fsf.x_selected ? SELCOL : x->x_gui.x_fcol, x->x_buf);
            sys_vgui(".x%lx.c itemconfigure %lxBASE1 -width %d\n",
                glist_getcanvas(glist), x, x->x_zoom);
            x->x_buf[0] = 0;
        }
    }
}
...
static void numbox_tilde_draw_new(t_numbox_tilde *x, t_glist *glist){
    ...
    sys_vgui(".x%lx.c create polygon %d %d %d %d %d %d %d %d %d %d "
             "-width %d -outline black -fill #%06x -tags [list %lxBASE1 %lxALL]\n",
        canvas,
        xpos, ypos,
        xpos + w, ypos,
        ...
        xpos, ypos,
        x->x_zoom, x->x_gui.x_bcol, x, x);
    sys_vgui(".x%lx.c create text %d %d -text {%s} -anchor w -font {{%s} -%d %s} -fill #%06x -tags [list %lxBASE2 %lxALL]\n",
        canvas, xpos + 2*x->x_zoom + 1, ypos + half + d, "~",
        x->x_gui.x_font, x->x_gui.x_fontsize * x->x_zoom,
        sys_fontweight, x->x_gui.x_fcol, x, x);
    sys_vgui(".x%lx.c create rectangle %d %d %d %d -fill black -tags [list %lxOUT %lxALL]\n",
        canvas,
        xpos, ypos + x->x_gui.x_h + x->x_zoom - ioh,
    ...
    sys_vgui(".x%lx.c create text %d %d -text {%s} -anchor w -font {{%s} -%d %s} -fill #%06x -tags [list %lxnumbox %lxALL]\n",
        canvas, xpos + half + 2*x->x_zoom + 3, ypos + half + d,
        x->x_buf, x->x_gui.x_font, x->x_gui.x_fontsize * x->x_zoom,
        sys_fontweight, x->x_gui.x_fcol, x, x);
}
...
//    if((x->x_sel = sel))
    if(sel){
        ...
        sys_vgui(".x%lx.c itemconfigure %lxnumbox -fill black\n", cv, x);
    }
}
...
static void numbox_tilde_draw_config(t_numbox_tilde* x, t_glist* glist){
    ...
    sys_vgui(".x%lx.c itemconfigure %lxnumbox -font {{%s} -%d %s} -fill #%06x \n",
        canvas, x, x->x_gui.x_font, x->x_gui.x_fontsize * x->x_zoom, sys_fontweight,
        (x->x_gui.x_fsf.x_selected ? SELCOL : x->x_gui.x_fcol));
    sys_vgui(".x%lx.c itemconfigure %lxBASE1 -fill #%06x\n", canvas,
        x, x->x_gui.x_bcol);
    sys_vgui(".x%lx.c itemconfigure %lxBASE2 -fill #%06x -font {{%s} -%d %s} \n", canvas, x,
        (x->x_gui.x_fsf.x_selected ? SELCOL : x->x_gui.x_fcol), x->x_gui.x_font, x->x_gui.x_fontsize * x->x_zoom, sys_fontweight);
}

static void numbox_tilde_draw(t_numbox_tilde *x, t_glist *glist, int mode){
    if(mode == IEM_GUI_DRAW_MODE_UPDATE)
        sys_queuegui(x, glist, numbox_tilde_draw_update);
    else if(mode == IEM_GUI_DRAW_MODE_NEW)
        numbox_tilde_draw_new(x, glist);
    else if(mode == IEM_GUI_DRAW_MODE_ERASE)
        numbox_tilde_draw_erase(x, glist);
    else if(mode == IEM_GUI_DRAW_MODE_CONFIG)
        numbox_tilde_draw_config(x, glist);
}
...
static int iemgui_getcolorarg(int index, int ac, t_atom*av){
    if(index < 0 || index >= ac)
        return(0);
    if(IS_A_FLOAT(av,index))
        return(atom_getfloatarg(index, ac, av));
...
static void numbox_tilde_dialog(t_numbox_tilde *x, t_symbol *s, int ac, t_atom *av){
    ...
    int bcol = (int)iemgui_getcolorarg(4, ac, av);
    int fcol = (int)iemgui_getcolorarg(5, ac, av);
    int min = (int)iemgui_getcolorarg(6, ac, av);
    int max = (int)iemgui_getcolorarg(7, ac, av);
    t_atom undo[9];
...
static void *numbox_tilde_new(t_symbol *s, int ac, t_atom *av){
    ...
    x->x_gui.x_draw = (t_iemfunptr)numbox_tilde_draw;
...
void numbox_tilde_setup(void){
    ...
    numbox_tilde_widgetbehavior.w_deletefn    = iemgui_delete;
    numbox_tilde_widgetbehavior.w_visfn       = iemgui_vis;
