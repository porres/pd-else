...
static void numbox_tilde_save(t_gobj *z, t_binbuf *b){
    ...
    binbuf_addv(b, "ssiisiiissfff", gensym("#X"), gensym("obj"), (int)x->x_obj.te_xpix,
        (int)x->x_obj.te_ypix, gensym("numbox~"), x->x_numwidth, (x->x_h/x->x_zoom)-5,
...
static void numbox_tilde_dialog(t_numbox_tilde *x, t_symbol *s, int ac, t_atom *av){
    s = NULL;
    post("numbox_tilde_dialog - ac = %d", ac);
    ...
    SETCOLOR(at+3, x->x_bcol);
    SETCOLOR(at+4, x->x_fcol);
