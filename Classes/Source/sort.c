...
typedef struct _sortdata{
    ...
    int		 d_direction; // 1: ascending, -1: descending
    t_atom  *d_buf;
...
static int sort_cmp(t_atom *a1, t_atom *a2){
...
static void sort_qsort(t_sort *x, t_atom *av1, t_atom *av2, int left, int right, float dir){
    ...
    for(int i = left+1; i <= right; i++){
        if((dir * sort_cmp(av1 + i, av1 + left)) < 0){
	...
	if(buf){
    	...
    	if(banged){
			...
			for(int i = 0; i < natoms; i++)
    			SETFLOAT(&buf2[i], i);
    		sort_qsort(x, buf, buf2, 0, natoms - 1, x->x_direction);
    		x->x_inbuf1.d_direction = x->x_direction;
            outlet_list(x->x_out2, &s_list, natoms, buf2);
            outlet_list(((t_object *)x)->ob_outlet, &s_list, natoms, buf);
		}
...
static void sort_doit(t_sort *x, int bang){
    int n = x->x_inbuf1.d_natoms;
    if(n){
        t_sortdata *d = &x->x_outbuf1;
        sort_sort(x, n, d->d_buf, bang);
    }
    else
...
static void sort_check_size(t_sort *x, t_sortdata *d, int ac){
    if(ac > d->d_size){
...
static void set_list(t_sort *x, t_sortdata *d, int ac, t_atom *av){
    sort_check_size(x, d, ac);
    d->d_natoms = ac > d->d_size ? d->d_size : ac;
    memcpy(d->d_buf, av, d->d_natoms * sizeof(*d->d_buf));
}

static void sort_list(t_sort *x, t_symbol *s, int ac, t_atom *av){
    s = NULL;
    set_list(x, &x->x_inbuf1, ac, av);
    sort_doit(x, 0);
}

static void set_any(t_sort *x, t_sortdata *d, t_symbol *s, int ac, t_atom *av){
    sort_check_size(x, d, ac);
    d->d_natoms = ac > d->d_size ? d->d_size : ac;
    SETSYMBOL(d->d_buf, s);
    if(ac > 1)
        memcpy(d->d_buf+1, av, (d->d_natoms - 1)*sizeof(*d->d_buf));
}

static void sort_anything(t_sort *x, t_symbol *s, int ac, t_atom *av){
    set_any(x, &x->x_inbuf1, s, ac+1, av);
    sort_doit(x, 0);
}
...
static void *sort_new(t_floatarg f){
    ...
    x->x_direction = f;
    floatinlet_new(&x->x_ob, &x->x_direction);
