...
static void *rec_new(t_floatarg f){
    ...
    if((tracks = getbytes(ntracks * sizeof(*tracks)))){
        ...
        if((x = (t_rec *)pd_new(rec_class))){
            ...
            x->x_ntracks = ntracks;
            x->x_tracks = tracks;
            for(id = 1; id <= ntracks; id++, tracks++){ // 1-based
                ...
                tp->tr_id = id;
                tp->tr_listed = 0;
                tp->tr_elsefilehandle = elsefile_new((t_pd *)tp, rec_track_readhook, rec_track_writehook);
                tp->tr_mode = REC_STOPMODE;
                tp->tr_muted = 0;
                tp->tr_restarted = 0;
                tp->tr_atdelta = 0;
...
void rec_setup(void){
    ...
    rec_class = class_new(gensym("rec"), (t_newmethod)rec_new, (t_method)rec_free, sizeof(t_rec), 0, A_DEFFLOAT, 0);
