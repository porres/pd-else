...
static void pitch_tilde_tolerance(t_pitch_tilde *x, t_floatarg a)
{
  if (a > 0) {
    ...
    post("pitchdetect~ tolerance set to %.2f",
...
void pitchdetect_tilde_setup (void)
{
  pitch_tilde_class = class_new (gensym ("pitchdetect~"),
      (t_newmethod)pitch_tilde_new,
      (t_method)pitch_tilde_del,
      sizeof (t_pitch_tilde),
      CLASS_DEFAULT, A_GIMME, 0);
  class_addmethod(pitch_tilde_class,
      (t_method)pitch_tilde_dsp,
      gensym("dsp"), 0);
  class_addmethod(pitch_tilde_class,
      (t_method)pitch_tilde_tolerance,
      gensym("tolerance"), A_DEFFLOAT, 0);
  class_addmethod(pitch_tilde_class,
      (t_method)pitch_tilde_tolerance,
      gensym("tol"), A_DEFFLOAT, 0);
  CLASS_MAINSIGNALIN(pitch_tilde_class,
      t_pitch_tilde, tolerance);
}
