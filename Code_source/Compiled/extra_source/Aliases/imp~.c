...
static void imp_dsp(t_imp *x, t_signal **sp){
    ...
    || (x->x_ch3 > 1 && x->x_ch3 != x->x_nchans)){
        ...
        pd_error(x, "[imp~]: channel sizes mismatch");
        return;
    }
    dsp_add(imp_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec);
}

static void imp_midi(t_imp *x, t_floatarg f){
    x->x_midi = (int)(f != 0);
}

static void imp_soft(t_imp *x, t_floatarg f){
    x->x_soft = (int)(f != 0);
}

static void imp_list(t_imp *x, t_symbol *s, int ac, t_atom * av){
    s = NULL;
    if(ac == 0)
        return;
    if(x->x_list_size != ac){
...
static void imp_set(t_imp *x, t_symbol *s, int ac, t_atom *av){
    s = NULL;
    if(ac != 2)
        return;
...
static void *imp_new(t_symbol *s, int ac, t_atom *av){
    s = NULL;
...
// Magic
    ...
    return(x);
errstate:
    post("[imp~]: improper args");
    return(NULL);
}

void imp_tilde_setup(void){
    imp_class = class_new(gensym("imp~"), (t_newmethod)imp_new, (t_method)imp_free,
        sizeof(t_imp), CLASS_MULTICHANNEL, A_GIMME, 0);
    class_addmethod(imp_class, nullfn, gensym("signal"), 0);
    class_addmethod(imp_class, (t_method)imp_dsp, gensym("dsp"), A_CANT, 0);
    class_addlist(imp_class, imp_list);
    class_addmethod(imp_class, (t_method)imp_soft, gensym("soft"), A_DEFFLOAT, 0);
    class_addmethod(imp_class, (t_method)imp_midi, gensym("midi"), A_DEFFLOAT, 0);
    class_addmethod(imp_class, (t_method)imp_set, gensym("set"), A_GIMME, 0);
