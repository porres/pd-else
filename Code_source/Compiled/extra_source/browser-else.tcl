...
proc menu_send_else_obj {w x y item} {
    if {$item eq "else"} {  
        ...
        set abslist {allpass.filt~  batch.rec~ batch.write~ bin.shift~ bl.osc~ bl.wavetable~ blip~ bpbank~ brickwall~ chorus~ compress~ crusher~ dispatch drunkard~ duck~ echo.rev~ envelope~ expand~ flanger~ float2sig~ free.rev~ freeze~ gain~ gain2~ gatehold~ grain.live~ grain.sampler~ grain.synth~ gran.player~ graph~ hann~ hip.bw~ lop.bw~ mag~ maxpeak~ meter~ meter2~ meter4~ meter8~ metronome~ mix2~ mix4~ mono.rev~ morph~ ms2samps~ noisegate~ norm~ oscbank~ oscbank2~ oscnoise~ out.mc~ out~ out4~ out8~ pan8~ perlin~ phaser~ ping.pong~ pitch.shift~ plate.rev~ play.file~ player~ polymetro~ pvoc.freeze~ pvoc.live~ pvoc.player~ rec.file~ resonbank~ resonbank2~ revdelay~ rm~ sample~ samps2ms~ setdsp~ spectrograph~ stereo.rev~ stretch.shift~ synth~ tremolo~ trig2bang~ vibrato~ vocoder~ voices~ above add amean any2symbol autotune autotune2 avg bangdiv biplot bpm brown car2pol cents2frac cents2scale chrono circle clock coeff2pz combine count db2lin dec2frac dec2hex deg2rad delete display drum.seq drunkard e eqdiv equal euclid frac.add frac.mul frac2cents frac2dec freq2midi glide glide2 gmean group hex2dec histogram impulse insert interpolate iterate keymap keypress lastvalue lcm lfnoise lfo lin2db list.inc list.seq mag makenote2 markov median meter midi.clock midi.in midi.learn midi.out midi2freq mono mono~ morph mov.avg ms2samps mtx.ctl multi.vsl nmess notedur note2midi op osc.receive osc.send pattern phasor pi pick pimp midi2note pol2car polymetro presets pulse pz2coeff rad2deg rampnoise rand.dist rand.list randpulse randpulse2 range.hsl range rec2 remove replace retune reverse rotate samps2ms scala scale2cents scale2freq scales schmitt score score2 scramble sendmidi sequencer slew slew2 slider2d speed stack status stepnoise store stream sum swap2 sysrt.in sysrt.out tabgen tap tempo timed.gate trig2bang unite zbiplot}
...
proc category_else_menu::load_menutree {} {
    set menutree { 
        {else
            ...
            {tuning/notes
                {scales scale2freq scala autotune autotune2 makenote2 retune eqdiv cents2scale scale2cents cents2frac frac2cents dec2frac frac2dec freq2midi midi2freq note2midi midi2note notedur}}
            {patch/subpatch\ management
                {loadbanger args meter presets dollsym sender receiver retrieve dispatch var send2~ blocksize~ nop~ click properties fontsize findfile canvas.active canvas.bounds canvas.gop canvas.pos canvas.edit canvas.vis canvas.name canvas.setname canvas.zoom}}
            {message\ management
            ...
            {control:\ fade\ pan\ routing
                {fader~ autofade~ autofade2~ balance~ pan2~ pan4~ pan8~ spread~ spread.mc~ rotate~ rotate.mc~ xfade~ xfade.mc~ xgate~ xgate.mc~ xgate2~ xgate2.mc~ xselect~ xselect2~ xselect2.mc~ xselect.mc~ mtx~}}
            {control:\ sequencers
