...
static void conv_tilde_window(t_conv_tilde *x, t_float w)
{
    ...
    x->x_non_overlapped_output = (t_sample *)t_resizebytes(
        x->x_non_overlapped_output,
        (2*x->x_window_double)*sizeof(t_sample),
        0
    );
...
static void conv_tilde_print(t_conv_tilde *x)
{
    ...
    post("%s: window size: %i", x->x_obj_symbol->s_name, x->x_window);
    post("%s: number of partitions: %i", x->x_obj_symbol->s_name, x->x_num_parts);
    post("%s: sampling rate: %i", x->x_obj_symbol->s_name, (int)x->x_sr);
    post("%s: block size: %i", x->x_obj_symbol->s_name, (int)x->x_n);
}


static void conv_tilde_flush(t_conv_tilde *x)
{
    int i;
...
static void *conv_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    //post("%s: version 0.13.1", x->x_obj_symbol->s_name);
    //post("%s: partition size %i", x->x_obj_symbol->s_name, x->x_window);
...
{
  conv_tilde_class =
      class_new(gensym("conv~"), (t_newmethod)conv_tilde_new,
                (t_method)conv_tilde_free, sizeof(t_conv_tilde),
                CLASS_DEFAULT, A_GIMME, 0);

  CLASS_MAINSIGNALIN(conv_tilde_class, t_conv_tilde, x_f);

  class_addmethod(conv_tilde_class, (t_method)conv_tilde_print,
                  gensym("print"), 0);

  class_addmethod(conv_tilde_class, (t_method)conv_tilde_window,
                  gensym("window"), A_DEFFLOAT, 0);

  class_addmethod(conv_tilde_class, (t_method)conv_tilde_analyze,
                  gensym("analyze"), A_SYMBOL, 0);

  class_addmethod(conv_tilde_class, (t_method)conv_tilde_eq,
                  gensym("eq"), A_GIMME, 0);

  class_addmethod(conv_tilde_class, (t_method)conv_tilde_flush,
                  gensym("flush"), 0);

  class_addmethod(conv_tilde_class, (t_method)conv_tilde_dsp,
                  gensym("dsp"), 0);
}
