...
t_int *circuit_tilde_perform(t_int *w)
{
    ...
    for(int i = 0; i < n; i++)
    {
        if(x->x_netlist && x->x_enabled)  {
            ...
            {
                set_input(x->x_netlist, in, ((t_sample *)w[in + 3])[i]);
            }
            ...
            {
                ((t_sample *)w[x->x_numin + out + 3])[i] = get_output(x->x_netlist, out);
            }
        }
        else {
            for(int out = 0; out < x->x_numout; out++)
...
void *circuit_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    x->x_netlist = create(argc, argv, 44100.0);
    x->x_numin = num_inlets(x->x_netlist);
    x->x_numout = num_outlets(x->x_netlist);
    x->x_enabled = 1;
...
void circuit_tilde_iter(t_circuit_tilde *x, t_float niter) {
    set_iter(x->x_netlist, (int)niter);
};
...
void circuit_tilde_dcblock(t_circuit_tilde *x, t_float dcblock) {
    set_dc_block(x->x_netlist, dcblock);
};
