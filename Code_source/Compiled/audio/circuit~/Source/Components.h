...
struct Inductor : Component<2, 1>
{
    ...
    void update(MNASystem& m) final
    {
        ...
        if(m.tStep != 0) g = 1. / ((2.*l) / (1. / m.tStep));
        else g = 0;
...
struct Probe : Component<2, 1>
{
    ...
    Probe(int l0, int l1, int outChannel) : outChannel(outChannel)
    {
        pinLoc[0] = l0;
        pinLoc[1] = l1;
    }
...
struct Potentiometer final : Component<3, 0>
{
    ...
    Potentiometer(double& position, double resistance, int vIn, int vOut, int vInvOut) : pos(position), r(resistance)
    {
        pinLoc[0] = vIn;
        pinLoc[1] = vOut;
        pinLoc[2] = vInvOut;
