...
static int start_paint(lua_State* L) {
    ...
    // Set graphics context as return value
...
static int fill_ellipse(lua_State* L) {
    ...
    plugdata_draw(ctx->object, gensym("lua_fill_ellipse"), 4, args);
    return 0;
}
...
static int stroke_ellipse(lua_State* L) {
    ...
    plugdata_draw(ctx->object, gensym("lua_stroke_ellipse"), 5, args);
    return 0;
}
...
static int fill_rect(lua_State* L) {
    ...
    SETFLOAT(args + 2, luaL_checknumber(L, 3)); // w
    SETFLOAT(args + 3, luaL_checknumber(L, 4)); // h
    plugdata_draw(ctx->object, gensym("lua_fill_rect"), 4, args);
    return 0;
}
...
static int stroke_rect(lua_State* L) {
    ...
    plugdata_draw(ctx->object, gensym("lua_stroke_rect"), 5, args);
    return 0;
}
...
static int fill_rounded_rect(lua_State* L) {
    ...
    plugdata_draw(ctx->object, gensym("lua_fill_rounded_rect"), 5, args);
    return 0;
}
...
static int stroke_rounded_rect(lua_State* L) {
    ...
    plugdata_draw(ctx->object, gensym("lua_stroke_rounded_rect"), 6, args);
    return 0;
}
...
static int draw_line(lua_State* L) {
    ...
    plugdata_draw(ctx->object, gensym("lua_draw_line"), 5, args);
...
static int draw_text(lua_State* L) {
    ...
    plugdata_draw(ctx->object, gensym("lua_draw_text"), 5, args);
    return 0;
}
...
static int stroke_path(lua_State* L) {
    ...
    plugdata_draw(ctx->object, gensym("lua_stroke_path"), 2, args);
    return 0;
}
...
static int fill_path(lua_State* L) {
    ...
    plugdata_draw(ctx->object, gensym("lua_fill_path"), 1, &args);
    return 0;
}
...
static void transform_size(t_graphics_context *ctx, int* w, int* h) {
    for(int i = ctx->num_transforms - 1; i >= 0; i--)
    {
        if(ctx->transforms[i].type == SCALE)
        {
            *w *= ctx->transforms[i].x;
            *h *= ctx->transforms[i].y;
        }
    }
}

static void transform_point(t_graphics_context *ctx, int* x, int* y) {
    for(int i = ctx->num_transforms - 1; i >= 0; i--)
    {
        if(ctx->transforms[i].type == SCALE)
        {
            *x *= ctx->transforms[i].x;
            *y *= ctx->transforms[i].y;
        }
        else // translate
        {
            *x += ctx->transforms[i].x;
...
static void get_bounds_args(lua_State* L, t_pdlua *obj, t_graphics_context *ctx, int* x1, int* y1, int* x2, int* y2) {
    t_canvas *cnv = glist_getcanvas(obj->canvas);
...
static const char* register_drawing(t_graphics_context *ctx)
{
    generate_random_id(ctx->current_item_tag, 64);
    return ctx->current_item_tag;
}
...
static int end_paint(lua_State* L) {
    ...
    t_pdlua_gfx *gfx = &obj->gfx;
    glist_drawiofor(glist_getcanvas(obj->canvas), (t_object*)obj, 1, ctx->object_tag, xpos, ypos, xpos + (obj->gfx.width * scale), ypos + (obj->gfx.height * scale));
    ...
    if(!gfx->first_draw && gfx->order_tag[0] != '\0') {
        ...
        pdgui_vmess(0, "crss", cnv, "lower", ctx->object_tag, gfx->order_tag);
    }
...
static int set_color(lua_State* L) {
    ...
    snprintf(ctx->current_color, 8, "#%02X%02X%02X", r, g, b);
    ctx->current_color[7] = '\0';
...
static int fill_ellipse(lua_State* L) {
    ...
    int x1, y1, x2, y2;
    get_bounds_args(L, obj, ctx, &x1, &y1, &x2, &y2);
    
    const char* tags[] = { ctx->object_tag, register_drawing(ctx) };
    
    pdgui_vmess(0, "crr iiii rs ri rS", cnv, "create", "oval", x1, y1, x2, y2, "-fill", ctx->current_color, "-width", 0, "-tags", 2, tags);
...
static int stroke_ellipse(lua_State* L) {
    ...
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "oval", x1, y1, x2, y2, "-width", line_width, "-outline", ctx->current_color, "-tags", 2, tags);
...
static int fill_all(lua_State* L) {
    ...
    pdgui_vmess(0, "crr iiii rs rS", cnv, "create", "rectangle", x1, y1, x2, y2, "-fill", ctx->current_color, "-tags", 2, tags);
...
static int fill_rect(lua_State* L) {
    ...
    pdgui_vmess(0, "crr iiii rs ri rS", cnv, "create", "rectangle", x1, y1, x2, y2, "-fill", ctx->current_color, "-width", 0, "-tags", 2, tags);
...
static int stroke_rect(lua_State* L) {
    ...
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "rectangle", x1, y1, x2, y2, "-width", line_width, "-outline", ctx->current_color, "-tags", 2, tags);
...
static int fill_rounded_rect(lua_State* L) {
    ...
    transform_size(ctx, &radius_x, &radius_y);
        
    const char* tags[] = { ctx->object_tag, register_drawing(ctx) };

    // Tcl/tk can't fill rounded rectangles, so we draw 2 smaller rectangles with 4 ovals over the corners
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "oval", x1, y1, x1 + radius_x * 2, y1 + radius_y * 2, "-width", 0, "-fill", ctx->current_color, "-tags", 2, tags);
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "oval", x2 - radius_x * 2 , y1, x2, y1 + radius_y * 2, "-width", 0, "-fill", ctx->current_color, "-tags", 2, tags);
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "oval", x1, y2 - radius_y * 2, x1 + radius_x * 2, y2, "-width", 0, "-fill", ctx->current_color, "-tags", 2, tags);
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "oval", x2 - radius_x * 2, y2 - radius_y * 2, x2, y2, "-width", 0, "-fill", ctx->current_color, "-tags", 2, tags);
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "rectangle", x1 + radius_x, y1, x2 - radius_x, y2, "-width", 0, "-fill", ctx->current_color, "-tag", 2, tags);
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "rectangle", x1, y1 + radius_y, x2, y2 - radius_y, "-width", 0, "-fill", ctx->current_color, "-tags", 2, tags);
...
static int stroke_rounded_rect(lua_State* L) {
    ...
    pdgui_vmess(0, "crr iiii ri ri ri ri rs rs rS", cnv, "create", "arc", x1, y1 + radius_y*2, x1 + radius_x*2, y1,
                "-start", 0, "-extent", 90, "-width", line_width, "-start", 90, "-outline", ctx->current_color, "-style", "arc", "-tags", 2, tags);
    pdgui_vmess(0, "crr iiii ri ri ri ri rs rs rS", cnv, "create", "arc", x2 - radius_x*2, y1, x2, y1 + radius_y*2,
                "-start", 270, "-extent", 90, "-width", line_width, "-start", 0, "-outline", ctx->current_color, "-style", "arc", "-tags", 2, tags);
    pdgui_vmess(0, "crr iiii ri ri ri ri rs rs rS", cnv, "create", "arc", x1, y2 - radius_y*2, x1 + radius_x*2, y2,
                "-start", 180, "-extent", 90, "-width", line_width, "-start", 180, "-outline", ctx->current_color, "-style", "arc", "-tags", 2, tags);
    pdgui_vmess(0, "crr iiii ri ri ri ri rs rs rS", cnv, "create", "arc", x2 - radius_x*2, y2, x2, y2 - radius_y*2,
                "-start", 90, "-extent", 90, "-width", line_width, "-start", 270, "-outline", ctx->current_color, "-style", "arc", "-tags", 2, tags);
    
    // Connect with lines
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "line", x1 + radius_x, y1, x2 - radius_x, y1,
                "-width", line_width, "-fill", ctx->current_color, "-tags", 2, tags);
...
static int draw_line(lua_State* L) {
    ...
    transform_point(ctx, &x1, &y1);
    transform_point(ctx, &x2, &y2);
...
static int stroke_path(lua_State* L) {
    ...
    pdgui_vmess(0, "crr iiii ri rs rS", cnv, "create", "line", 0, 0, 0, 0, "-width", stroke_width, "-fill", ctx->current_color, "-tags", 2, tags);
...
static int fill_path(lua_State* L) {
    ...
    pdgui_vmess(0, "crr r ri rs rS", cnv, "create", "polygon", coordinates, "-width", 0, "-fill", ctx->current_color, "-tags", 2, tags);
...
static int translate(lua_State* L) {
    ...
    if(ctx->num_transforms == 0)
    {
        ctx->transforms = getbytes(sizeof(gfx_transform));
        
    }
    else
    {
        ctx->transforms = resizebytes(ctx->transforms, ctx->num_transforms * sizeof(gfx_transform), (ctx->num_transforms + 1) * sizeof(gfx_transform));
        
    }
    
    ctx->transforms[ctx->num_transforms].type = TRANSLATE;
    ctx->transforms[ctx->num_transforms].x = luaL_checknumber(L, 1);
    ctx->transforms[ctx->num_transforms].y = luaL_checknumber(L, 2);
...
static int scale(lua_State* L) {
    ...
    ctx->transforms[ctx->num_transforms].type = SCALE;
    ctx->transforms[ctx->num_transforms].x = luaL_checknumber(L, 1);
    ctx->transforms[ctx->num_transforms].y = luaL_checknumber(L, 2);
