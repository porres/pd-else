        ...
        for(int i = 0, n = x->x_n; i < n; i++){
            ...
            if(hz >= 0){ // POSITIVE freq
                ...
                if(phase[j] >= 1){ // 1st is always 1
                    ...
                    out[j*n + i] = phase[j] < width ? 1 : -1;
            }
            else{ // negative freq
                if(phase[j] >= 1)
                    phase[j] -= 1.;
                if(phase[j] <= 0){
                    out[j*n + i] = -1; // 1st sample is always -1
                    phase[j] += 1;
                }
...
static void square_dsp(t_square *x, t_signal **sp){
    ...
    || (x->x_ch4 > 1 && x->x_ch4 != x->x_nchans)){
        ...
        pd_error(x, "[square~]: channel sizes mismatch");
        return;
    }
    dsp_add(square_perform, 6, x, sp[0]->s_vec, sp[1]->s_vec,
        sp[2]->s_vec, sp[3]->s_vec, sp[4]->s_vec);
}

static void square_midi(t_square *x, t_floatarg f){
    x->x_midi = (int)(f != 0);
}

static void square_soft(t_square *x, t_floatarg f){
    x->x_soft = (int)(f != 0);
}

static void square_list(t_square *x, t_symbol *s, int ac, t_atom * av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    if(x->x_list_size != ac){
...
static void square_set(t_square *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac != 2)
        return;
...
// Magic
    ...
    return(x);
errstate:
    post("[square~]: improper args");
    return(NULL);
}

void square_tilde_setup(void){
    square_class = class_new(gensym("square~"), (t_newmethod)square_new, (t_method)square_free,
        sizeof(t_square), CLASS_MULTICHANNEL, A_GIMME, 0);
    class_addmethod(square_class, nullfn, gensym("signal"), 0);
    class_addmethod(square_class, (t_method)square_dsp, gensym("dsp"), A_CANT, 0);
    class_addlist(square_class, square_list);
    class_addmethod(square_class, (t_method)square_soft, gensym("soft"), A_DEFFLOAT, 0);
    class_addmethod(square_class, (t_method)square_midi, gensym("midi"), A_DEFFLOAT, 0);
    class_addmethod(square_class, (t_method)square_set, gensym("set"), A_GIMME, 0);
}
