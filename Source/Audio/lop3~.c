...
static t_class *lop3_class;

static void update_coeffs(t_lop3 *x, int j, double f){
    x->x_f[j] = f;
    if(f <= 0){
    ...
    x->x_a0[j] = x->x_a1[j] = 0.5 * k;
    x->x_b1[j] = 1.0 - k;
}

static t_int *lop3_perform(t_int *w){
    t_lop3 *x = (t_lop3 *)(w[1]);
...
static void lop3_dsp(t_lop3 *x, t_signal **sp){
    x->x_n = sp[0]->s_n;
    ...
    if(x->x_ch2 > 1 && x->x_ch2 != x->x_nchans){
        ...
        pd_error(x, "[lop3~]: channel sizes mismatch");
        return;
    }
    dsp_add(lop3_perform, 4, x, sp[0]->s_vec,
        sp[1]->s_vec, sp[2]->s_vec);
}

static void lop3_clear(t_lop3 *x){
    for(int i = 0; i < x->x_nchans; i++)
        x->x_xnm1[i] = x->x_ynm1[i] = 0;
}

static void *lop3_free(t_lop3 *x){
...
static void *lop3_new(t_floatarg f){
    t_lop3 *x = (t_lop3 *)pd_new(lop3_class);
...
void lop3_tilde_setup(void){
    lop3_class = class_new(gensym("lop3~"), (t_newmethod)lop3_new,
        (t_method)lop3_free, sizeof(t_lop3), CLASS_MULTICHANNEL, A_DEFFLOAT, 0);
    class_addmethod(lop3_class, (t_method)lop3_dsp, gensym("dsp"), A_CANT, 0);
    class_addmethod(lop3_class, nullfn, gensym("signal"), 0);
    class_addmethod(lop3_class, (t_method)lop3_clear, gensym("clear"), 0);
}
