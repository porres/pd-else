...
double parabolic_wrap_phase(double phase){
    while(phase >= 1)
        phase -= 1.;
    while(phase < 0)
        phase += 1.;
...
// Magic End
    for(int j = 0; j < x->x_nchans; j++){
        for(int i = 0, n = x->x_n; i < n; i++){
            ...
            out[j*n + i] = read_partab(parabolic_wrap_phase(phase[j] + phase_offset));
            phase[j] = parabolic_wrap_phase(phase[j] + step);
        }
    }
    x->x_phase = phase;
    x->x_dir = dir;
...
static void parabolic_dsp(t_parabolic *x, t_signal **sp){
    ...
    || (x->x_ch3 > 1 && x->x_ch3 != x->x_nchans)){
        ...
        pd_error(x, "[parabolic~]: channel sizes mismatch");
        return;
    }
    dsp_add(parabolic_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec);
}

static void parabolic_midi(t_parabolic *x, t_floatarg f){
    x->x_midi = (int)(f != 0);
}

static void parabolic_set(t_parabolic *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac != 2)
        return;
...
static void parabolic_list(t_parabolic *x, t_symbol *s, int ac, t_atom * av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    if(x->x_list_size != ac){
...
static void parabolic_soft(t_parabolic *x, t_floatarg f){
    x->x_soft = (int)(f != 0);
}
...
// Magic
    ...
    return(x);
errstate:
    post("[parabolic~]: improper args");
    return(NULL);
}

void parabolic_tilde_setup(void){
    parabolic_class = class_new(gensym("parabolic~"), (t_newmethod)parabolic_new, (t_method)parabolic_free,
        sizeof(t_parabolic), CLASS_MULTICHANNEL, A_GIMME, 0);
    class_addmethod(parabolic_class, nullfn, gensym("signal"), 0);
    class_addmethod(parabolic_class, (t_method)parabolic_dsp, gensym("dsp"), A_CANT, 0);
    class_addlist(parabolic_class, parabolic_list);
