...
static void *changed_new(t_floatarg f1, t_floatarg f2){
    t_changed *x = (t_changed *)pd_new(changed_class);
    x->x_thresh = f1 < 0 ? 0 : f1;
    x->x_x1m1 = f2;
    x->x_tresh_inlet = inlet_new((t_object *)x, (t_pd *)x, &s_signal, &s_signal);
    pd_float((t_pd *)x->x_tresh_inlet, f1);
...
void changed_tilde_setup(void){
    changed_class = class_new(gensym("changed~"), (t_newmethod)changed_new,
        (t_method)changed_free, sizeof(t_changed), CLASS_DEFAULT, A_DEFFLOAT, A_DEFFLOAT, 0);
