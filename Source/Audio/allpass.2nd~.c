...
static void allpass_2nd_freq(t_allpass_2nd *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    for(int i = 0; i < ac; i++)
...
static void allpass_2nd_reson(t_allpass_2nd *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    for(int i = 0; i < ac; i++)
    ...
    for(int j = 0; j < x->x_nchans; j++){
        for(int i = 0, n = x->x_n; i < n; i++){
            ...
            if(x->x_bypass)
                ...
                yn = x->x_a0 * xn + x->x_a1 * xnm1[j] + xnm2[j] + x->x_b1 * ynm1[j] + x->x_b2 * ynm2[j];
                out[j*n + i] = yn;
                xnm2[j] = xnm1[j];
                xnm1[j] = xn;
                ynm2[j] = ynm1[j];
...
static void allpass_2nd_dsp(t_allpass_2nd *x, t_signal **sp){
    ...
    || (x->x_ch3 > 1 && x->x_ch3 != x->x_nchans)){
        ...
        pd_error(x, "[allpass.2nd~]: channel sizes mismatch");
        return;
    }
    dsp_add(allpass_2nd_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec,
...
static void *allpass_2nd_free(t_allpass_2nd *x){
...
static void *allpass_2nd_new(t_symbol *s, int ac, t_atom *av){
...
void setup_allpass0x2e2nd_tilde(void){
    allpass_2nd_class = class_new(gensym("allpass.2nd~"), (t_newmethod)allpass_2nd_new,
        (t_method)allpass_2nd_free, sizeof(t_allpass_2nd), CLASS_MULTICHANNEL, A_GIMME, 0);
    ...
    class_addmethod(allpass_2nd_class, (t_method)allpass_2nd_freq, gensym("freq"), A_GIMME, 0);
    class_addmethod(allpass_2nd_class, (t_method)allpass_2nd_reson, gensym("reson"), A_GIMME, 0);
}
