    ...
    for(int j = 0; j < x->x_nchans; j++){
        for(int i = 0, n = x->x_n; i < n; i++){
            ...
            if(x->x_sig3){
                ...
                if(x->x_ch3 == 1)
                    reson = x->x_reson_list[0];
                else
                    reson = x->x_reson_list[j];
            }
            if(f < 0.000001)
                f = 0.000001;
            if(f > nyq - 0.000001)
...
static void bandpass_dsp(t_bandpass *x, t_signal **sp){
    ...
    dsp_add(bandpass_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec,
...
void bandpass_tilde_setup(void){
    bandpass_class = class_new(gensym("bandpass~"), (t_newmethod)bandpass_new,
        (t_method)bandpass_free, sizeof(t_bandpass), CLASS_MULTICHANNEL, A_GIMME, 0);
    ...
    class_addmethod(bandpass_class, (t_method)bandpass_freq, gensym("freq"), A_GIMME, 0);
    class_addmethod(bandpass_class, (t_method)bandpass_reson, gensym("reson"), A_GIMME, 0);
}
