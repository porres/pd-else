...
static t_int *del_out_perform(t_int *w){
    ...
    t_sample *vp = ctl->c_vec + ch * (nsamps + XTRASAMPS);  // channel offset into delay buffer
    t_sample *bp;
    t_sample *wp = vp + ctl->c_phase;                        // wp (write point) = vp + phase
    t_sample zerodel = x->x_zerodel;
    if(limit < 0){      // blocksize is larger than out buffer size
        for(int i = 0; i < n; i++)
            out[i] = 0; // output zeros
        return(w+7);
    }
    for(int i = 0; i < n; i++){
        t_sample samps = in[i];      // delay in samples = input
        ...
        nm1--;                                    // per-sample decrement
        int isamps = (int)samps;                  // delay point integer part
        t_sample frac = samps - (t_sample)isamps; // fractional part
        bp = wp - isamps;                         // buffer point = write point - delay
        if(bp < vp + XTRASAMPS)                   // wrap
            bp += nsamps;
        t_sample a = bp[0], b = bp[-1], c = bp[-2], d = bp[-3];
        out[i] = interp_spline(frac, a, b, c, d); // MC output write
    }
    return(w+7);
}
...
static void del_out_dsp(t_del_out *x, t_signal **sp){
    ...
    x->x_nchans = delwriter ? delwriter->x_nchans : 1;
...
//        dsp_add(del_out_perform, 5, sp[0]->s_vec, sp[1]->s_vec, &delwriter->x_cspace, x, (t_int)sp[0]->s_n);
        for(int ch = 0; ch < delwriter->x_nchans; ch++){
            dsp_add(del_out_perform, 6,
                sp[0]->s_vec,
                sp[1]->s_vec + ch * sp[1]->s_n,
                &delwriter->x_cspace,
                x,
                ch,
                (t_int)sp[1]->s_n);
...
void setup_del0x2eout_tilde(void){
    del_out_class = class_new(gensym("del.out~"), (t_newmethod)del_out_new, 0, sizeof(t_del_out), CLASS_MULTICHANNEL, A_GIMME, 0);
    CLASS_MAINSIGNALIN(del_out_class, t_del_out, x_f);
    class_addmethod(del_out_class, nullfn, gensym("signal"), 0);
