...
static void henon_coeffs(t_henon *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    int argnum = 0; // current argument
    while(ac){
        if(av->a_type != A_FLOAT)
            pd_error(x, "henon~: coefficients must be floats");
        else{
...
static t_int *henon_perform(t_int *w){
    ...
    for(int j = 0; j < x->x_nchans; j++){
        for(int i = 0, n = x->x_n; i < n; i++){
            ...
            if(trig){ // update
                output = 1 - (a * ynm1[j]*ynm1[j]) + (b * ynm2[j]);
                if(fabs(output) > 1.5){
                    output = 0;
                    ynm2[j] = ynm1[j] = 0;
                }
...
static void henon_dsp(t_henon *x, t_signal **sp){
    ...
    dsp_add(henon_perform, 4, x, nchans, sp[0]->s_vec, sp[1]->s_vec);
}

static void henon_list(t_henon *x, t_symbol *s, int ac, t_atom * av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    for(int i = 0; i < ac; i++)
...
static void henon_set(t_henon *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac != 2)
        return;
...
static void *henon_new(t_symbol *s, int ac, t_atom *av){
    ...
    return(x);
errstate:
    post("[henon~]: improper args");
    return(NULL);
}

void henon_tilde_setup(void){
    henon_class = class_new(gensym("henon~"), (t_newmethod)henon_new, (t_method)henon_free, sizeof(t_henon), CLASS_MULTICHANNEL, A_GIMME, 0);
    class_addmethod(henon_class, nullfn, gensym("signal"), 0);
    ...
    class_addmethod(henon_class, (t_method)henon_coeffs, gensym("coeffs"), A_GIMME, 0);
}
