...
static void impulse2_dsp(t_impulse2 *x, t_signal **sp){
    ...
    || (x->x_ch4 > 1 && x->x_ch4 != x->x_nchans)){
        ...
        pd_error(x, "[impulse2~]: channel sizes mismatch");
        return;
    }
    dsp_add(impulse2_perform, 6, x, sp[0]->s_vec, sp[1]->s_vec,
        sp[2]->s_vec, sp[3]->s_vec, sp[4]->s_vec);
}

static void impulse2_midi(t_impulse2 *x, t_floatarg f){
    x->x_midi = (int)(f != 0);
}

static void impulse2_soft(t_impulse2 *x, t_floatarg f){
    x->x_soft = (int)(f != 0);
}

static void impulse2_list(t_impulse2 *x, t_symbol *s, int ac, t_atom * av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    if(x->x_list_size != ac){
...
static void impulse2_set(t_impulse2 *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac != 2)
        return;
...
// Magic
    ...
    return(x);
errstate:
    post("[impulse2~]: improper args");
    return(NULL);
}

void impulse2_tilde_setup(void){
    impulse2_class = class_new(gensym("impulse2~"), (t_newmethod)impulse2_new, (t_method)impulse2_free,
        sizeof(t_impulse2), CLASS_MULTICHANNEL, A_GIMME, 0);
    class_addmethod(impulse2_class, nullfn, gensym("signal"), 0);
    class_addmethod(impulse2_class, (t_method)impulse2_dsp, gensym("dsp"), A_CANT, 0);
    class_addlist(impulse2_class, impulse2_list);
    class_addmethod(impulse2_class, (t_method)impulse2_soft, gensym("soft"), A_DEFFLOAT, 0);
    class_addmethod(impulse2_class, (t_method)impulse2_midi, gensym("midi"), A_DEFFLOAT, 0);
    class_addmethod(impulse2_class, (t_method)impulse2_set, gensym("set"), A_GIMME, 0);
}
