...
static void cosine_dsp(t_cosine *x, t_signal **sp){
    ...
    dsp_add(cosine_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec);
}
...
static void cosine_set(t_cosine *x, t_symbol *s, int ac, t_atom *av){
    s = NULL;
    if(ac != 2)
        return;
...
static void cosine_list(t_cosine *x, t_symbol *s, int ac, t_atom * av){
    s = NULL;
    if(ac == 0)
        return;
    if(x->x_list_size != ac){
...
void cosine_tilde_setup(void){
    ...
    class_addlist(cosine_class, cosine_list);
