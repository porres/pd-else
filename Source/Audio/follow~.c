...
static void follow_dsp(t_follow *x, t_signal **sp){
    ...
    if((ch2 > 1 && ch2 != x->x_nchans) || (ch3 > 1 && ch3 != x->x_nchans)){
        ...
        pd_error(x, "[follow~]: channel sizes mismatch");
    }
    else
        dsp_add(follow_perform, 8, x, n, ch2, ch3,
...
static void *follow_new(t_floatarg f1, t_floatarg f2){
...
void follow_tilde_setup(void){
    follow_class = class_new(gensym("follow~"), (t_newmethod)follow_new, (t_method)follow_free,
        sizeof(t_follow), CLASS_MULTICHANNEL, A_DEFFLOAT, A_DEFFLOAT, 0);
    CLASS_MAINSIGNALIN(follow_class, t_follow, x_f);
