...
double saw_wrap_phase(double phase){
    while(phase >= 1)
        phase -= 1.;
    while(phase < 0)
        phase += 1.;
...
// Magic End
    for(int j = 0; j < x->x_nchans; j++){
        for(int i = 0, n = x->x_n; i < n; i++){
            ...
            t_float wrap = saw_wrap_phase(phase[j] + phase_offset);
            out[j*n + i] = wrap * -2.0f + 1.0f;
            phase[j] = saw_wrap_phase(phase[j] + step);
        }
    }
    x->x_phase = phase;
    x->x_dir = dir;
...
static void saw_dsp(t_saw *x, t_signal **sp){
    ...
    || (x->x_ch3 > 1 && x->x_ch3 != x->x_nchans)){
        ...
        pd_error(x, "[saw~]: channel sizes mismatch");
        return;
    }
    dsp_add(saw_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec);
}

static void saw_midi(t_saw *x, t_floatarg f){
    x->x_midi = (int)(f != 0);
}

static void saw_set(t_saw *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac != 2)
        return;
...
static void saw_list(t_saw *x, t_symbol *s, int ac, t_atom * av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    if(x->x_list_size != ac){
...
static void saw_soft(t_saw *x, t_floatarg f){
    x->x_soft = (int)(f != 0);
}
...
// Magic
    ...
    return(x);
errstate:
    post("[saw~]: improper args");
    return(NULL);
}

void saw_tilde_setup(void){
    saw_class = class_new(gensym("saw~"), (t_newmethod)saw_new, (t_method)saw_free,
        sizeof(t_saw), CLASS_MULTICHANNEL, A_GIMME, 0);
    class_addmethod(saw_class, nullfn, gensym("signal"), 0);
    class_addmethod(saw_class, (t_method)saw_dsp, gensym("dsp"), A_CANT, 0);
    class_addlist(saw_class, saw_list);
    class_addmethod(saw_class, (t_method)saw_soft, gensym("soft"), A_DEFFLOAT, 0);
    class_addmethod(saw_class, (t_method)saw_midi, gensym("midi"), A_DEFFLOAT, 0);
    class_addmethod(saw_class, (t_method)saw_set, gensym("set"), A_GIMME, 0);
}
