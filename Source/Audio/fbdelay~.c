...
static void fbdelay_clear(t_fbdelay *x){
    memset(x->x_ybuf, 0, x->x_nchans * x->x_sz * sizeof(*x->x_ybuf));
    for(unsigned int i = 0; i < x->x_nchans; i++)
        x->x_wp[i] = 0;
}
...
static void fbdelay_sz(t_fbdelay *x){
    ...
    newsz++;
    int alloc = x->x_alloc;
    unsigned int cursz = x->x_sz;
    if(newsz < 1)
        newsz = 1;
    else if(newsz > FBD_MAXD)
        newsz = FBD_MAXD;
    if(!alloc && newsz > FBD_STACK){
        x->x_ybuf = (double *)malloc(sizeof(double) * x->x_nchans * newsz);
        x->x_alloc = 1;
        x->x_sz = newsz;
    }
    else if(alloc && newsz > cursz){
        x->x_ybuf = (double *)realloc(x->x_ybuf,
            sizeof(double) * x->x_nchans * newsz);
        x->x_sz = newsz;
    }
...
static void fbdelay_nchans(t_fbdelay *x, unsigned int nchans){
    if(nchans < 1)
        nchans = 1;
    ...
    x->x_nchans = nchans;
    x->x_alloc = 0;
    x->x_fbstack = (double *)malloc(sizeof(double) * x->x_nchans * FBD_STACK);
    x->x_ybuf = x->x_fbstack;
    x->x_wp = (unsigned int *)malloc(sizeof(unsigned int) * x->x_nchans);
    fbdelay_sz(x);
}
...
static t_int *fbdelay_perform(t_int *w){
    ...
    unsigned int dnchans = (unsigned int)(w[7]);
    unsigned int anchan = (unsigned int)(w[8]);

    for(unsigned int ch = 0; ch < x->x_nchans; ch++){
        t_float *in = xin + ch * n;
        t_float *delin = din + (dnchans > 1 ? ch : 0) * n;
        t_float *fb = ain + (anchan > 1 ? ch : 0) * n;
        t_float *outch = out + ch * n;
        double *ybuf = x->x_ybuf + ch * x->x_sz;
        unsigned int wp = x->x_wp[ch];
        ...
        for(t_int i = 0; i < n; i++){
            ...
            if(!x->x_gain)
                fb[i] = fb[i] == 0 ? 0 :
                    copysign(exp(log(0.001) * ms / fabs(fb[i])), fb[i]);

            double output = (double)in[i] + y_n * (double)fb[i];
...
static void fbdelay_dsp(t_fbdelay *x, t_signal **sp){
    ...
    unsigned int nchans = sp[0]->s_nchans;
    if(sp[1]->s_nchans > nchans)
        nchans = sp[1]->s_nchans;
    if(sp[2]->s_nchans > nchans)
        nchans = sp[2]->s_nchans;

    if(sr_khz != x->x_sr_khz){
        x->x_sr_khz = sr_khz;
        fbdelay_sz(x);
    }
    fbdelay_nchans(x, nchans);

    signal_setmultiout(&sp[3], nchans);
    dsp_add(fbdelay_perform, 8, x, sp[0]->s_n, sp[0]->s_vec,
        sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec,
        sp[1]->s_nchans, sp[2]->s_nchans);
}
...
static void *fbdelay_new(t_symbol *s, int argc, t_atom *argv){
    ...
    x->x_nchans = 1;
    x->x_sz = FBD_STACK;
    x->x_fbstack = (double *)malloc(sizeof(double) * FBD_STACK);
    x->x_ybuf = x->x_fbstack;
    x->x_wp = (unsigned int *)malloc(sizeof(unsigned int));
    fbdelay_clear(x);
    float del_time = 0;
    float delsize = 1000;
    float fb = 0;
    ...
    x->x_dellet = inlet_new((t_object *)x, (t_pd *)x,
        &s_signal, &s_signal);
    pd_float((t_pd *)x->x_dellet, del_time);
    x->x_alet = inlet_new((t_object *)x, (t_pd *)x,
        &s_signal, &s_signal);
...
void fbdelay_tilde_setup(void){
    fbdelay_class = class_new(gensym("fbdelay~"),
        (t_newmethod)fbdelay_new, (t_method)fbdelay_free,
        sizeof(t_fbdelay), CLASS_MULTICHANNEL, A_GIMME, 0);
    class_addmethod(fbdelay_class, nullfn, gensym("signal"), 0);
    class_addmethod(fbdelay_class, (t_method)fbdelay_dsp,
        gensym("dsp"), A_CANT, 0);
    class_addmethod(fbdelay_class, (t_method)fbdelay_clear,
        gensym("clear"), 0);
    class_addmethod(fbdelay_class, (t_method)fbdelay_size,
        gensym("size"), A_FLOAT, 0);
    class_addmethod(fbdelay_class, (t_method)fbdelay_gain,
        gensym("gain"), A_FLOAT, 0);
    class_addmethod(fbdelay_class, (t_method)fbdelay_freeze,
        gensym("freeze"), A_FLOAT, 0);
}