...
static void quad_coeffs(t_quad *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    int argnum = 0; // current argument
    while(ac){
        if(av->a_type != A_FLOAT)
            pd_error(x, "quad~: coefficients must be floats");
        else{
...
static t_int *quad_perform(t_int *w){
    ...
    double a = x->x_a, b = x->x_b, c = x->x_c;
...
static void quad_dsp(t_quad *x, t_signal **sp){
    ...
    dsp_add(quad_perform, 4, x, nchans, sp[0]->s_vec, sp[1]->s_vec);
}

static void quad_list(t_quad *x, t_symbol *s, int ac, t_atom * av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    for(int i = 0; i < ac; i++)
...
static void quad_set(t_quad *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac != 2)
        return;
...
static void *quad_new(t_symbol *s, int ac, t_atom *av){
    ...
    return(x);
errstate:
    post("[quad~]: improper args");
    return(NULL);
}

void quad_tilde_setup(void){
    quad_class = class_new(gensym("quad~"), (t_newmethod)quad_new,
        (t_method)quad_free, sizeof(t_quad), CLASS_MULTICHANNEL, A_GIMME, 0);
    class_addmethod(quad_class, nullfn, gensym("signal"), 0);
    class_addmethod(quad_class, (t_method)quad_dsp, gensym("dsp"), A_CANT, 0);
    class_addlist(quad_class, quad_list);
    class_addmethod(quad_class, (t_method)quad_set, gensym("set"), A_GIMME, 0);
    class_addmethod(quad_class, (t_method)quad_coeffs, gensym("coeffs"), A_GIMME, 0);
}
