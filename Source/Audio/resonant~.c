...
static void resonant_freq(t_resonant *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    for(int i = 0; i < ac; i++)
...
static void resonant_reson(t_resonant *x, t_symbol *s, int ac, t_atom *av){
    x->x_ignore = s;
    if(ac == 0)
        return;
    for(int i = 0; i < ac; i++)
...
static void resonant_dsp(t_resonant *x, t_signal **sp){
    ...
    || (x->x_ch3 > 1 && x->x_ch3 != x->x_nchans)){
        ...
        pd_error(x, "[resonant~]: channel sizes mismatch");
        return;
    }
    dsp_add(resonant_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec,
...
static void resonant_bw(t_resonant *x){
    x->x_resmode = 1;
    update_coeffs(x, x->x_f, x->x_reson);
}
...
static void *resonant_free(t_resonant *x){
...
static void *resonant_new(t_symbol *s, int ac, t_atom *av){
...
void resonant_tilde_setup(void){
    resonant_class = class_new(gensym("resonant~"), (t_newmethod)resonant_new,
        (t_method)resonant_free, sizeof(t_resonant), CLASS_MULTICHANNEL, A_GIMME, 0);
    ...
    class_addmethod(resonant_class, (t_method)resonant_bw, gensym("bw"), 0);
    class_addmethod(resonant_class, (t_method)resonant_t60, gensym("t60"), 0);
    class_addmethod(resonant_class, (t_method)resonant_freq, gensym("freq"), A_GIMME, 0);
    class_addmethod(resonant_class, (t_method)resonant_reson, gensym("reson"), A_GIMME, 0);
}
