...
proc menu_send_else_obj {w x y item} {
    if {$item eq "else"} {  
        ...
        set abslist {abs.pd~ allpass.filt~ float2imp~ batch.rec~ batch.write~ bin.shift~ bl.osc~ bl.wavetable~ sortnote superosc~ blip~ bpbank~ brickwall~ chorus~ compress~ crush~ dir damp.osc~ dispatch drunkard~ duck~ echo.rev~ envelope~ expand~ flanger~ float2sig~ free.rev~ freeze~ filter gain~ gain2~ level~ gatehold gatehold~ gaterelease gatedelay gaterelease~ grain.live~ grain.sampler~ grain.synth~ gran.player~ graph~ hann~ hip.bw~ intervals lop.bw~ mag~ maxpeak~ meter~ meter2~ meter4~ meter8~ metronome~ mix2~ mix4~ mono.rev~ morph~ ms2samps~ noisegate~ norm~ notes.on oscbank~ oscnoise~ out.mc~ out~ out4~ out8~ perlin~ phaser~ ping.pong~ pitch.shift~ plate.rev~ play.file~ player~ polymetro~ pvoc.freeze~ pvoc.live~ pvoc.player~ rec.file~ reorder resonbank~ resonbank2~ revdelay~ rm~ sample~ samps2ms~ setdsp~ spectrograph~ stereo.rev~ stretch.shift~ synth~ tremolo~ trig2bang~ vibrato~ vocoder~ voices~ above add amean any2symbol autotune autotune2 avg bangdiv biplot bpm brown car2pol cents2frac cents2scale chrono circle clock coeff2pz combine count db2lin dec2frac dec2hex deg2rad delete display drum.seq drunkard e eqdiv equal arpeggiator euclid frac.add frac.mul frac2cents frac2dec freq2midi glide glide2 smooth smooth2 gmean group hex2dec histogram impulse insert interpolate iterate keymap keypress lastvalue lcm lfnoise lfo lin2db rand.dev list.harm list.inc list.seq mag makenote2 markov median meter midi.clock midi.in midi.learn midi.out midi2freq mono mono~ morph mov.avg ms2samps mtx.ctl multi.vsl nmess notedur2ratio note2midi op osc.receive osc.send pattern phasor pi pick pimp midi2note pol2car polymetro presets pulse pz2coeff rad2deg rampnoise rand.dist rand.list randpulse randpulse2 range.hsl range rec2 remove replace retune reverse rotate samps2ms scala scale2cents scale2freq scales schmitt score score2 scramble sendmidi sequencer slew slew2 slider2d speed stack status stepnoise store stream sum swap2 sysrt.in sysrt.out tabgen tap tempo timed.gate trig2bang unite zbiplot vca~ vca2~ ms.enc~ ms.dec~ formfm~ packet~ gatemin~ gatemin filter intervals sortnote notes.on reorder lsystem}
...
proc category_else_menu::load_else_menutree {} {
    set menutree { 
        ...
        {tuning/notes
            {intervals notes.on sortnote scales scale2freq scala autotune autotune2 makenote2 retune eqdiv cents2scale scale2cents cents2frac frac2cents dec2frac frac2dec freq2midi midi2freq note2midi midi2note notedur2ratio}}
        ...
        {list\ management
            {break order combine delete remove equal filter reorder listspread group iterate insert scramble sort reverse rotate replace sum slice stream merge unmerge unique amean gmean list.inc rand.dev list.harm lace delace}}
        {file\ management
            {dir}}
        {midi
        ...
        {data/signal\ control
            ...
            {triggers
                {above above~ bangdiv chance chance~ dust~ dust2~ gatehold gatehold~ gaterelease gaterelease~ gatemin~ gatemin gatedelay~ gatedelay gate2imp~ pimp~ pimpmul~ pulsecount~ pulsediv~ sh~ schmitt schmitt~ status status~ trig.delay~ toggleff~ timed.gate timed.gate~ match~ trig2bang trig2bang~ trighold~}}
            {triggers\ clock
