...
proc category_else_menu::load_else_menutree {} {
    set menutree { 
        ...
        {patch/subpatch\ management
            {loadbanger closebang args meter presets dollsym sender receiver retrieve dispatch var send2~ blocksize~ nop~ click properties fontsize canvas.active canvas.bounds canvas.gop canvas.pos canvas.edit canvas.vis canvas.name canvas.setname canvas.zoom sendmidi abs.pd~}}
        {message\ management
        ...
        {list\ management
            {break order combine delete remove equal filter reorder group iterate insert scramble sort reverse rotate replace sum slice stream merge unmerge unique amean gmean list.inc rand.dev list.harm lace delace}}
        {file\ management
            {dir}}
        {midi
        ...
        {analysis
            {changed~ changed2~ detect~ lastvalue~ median~ tap range range~ maxpeak~ follow~ peak~ rms~ mov.rms~ vu~ zerocross~ beat~}}
    }
    return $menutree
}
