...
static void knob_config_arc(t_knob *x){
    ...
    pdgui_vmess(0, "crs rs", cv, "itemconfigure", x->x_tag_arc,
        "-state", x->x_arc && !x->x_transparent && x->x_fval != x->x_arcstart ? "normal" : "hidden");
...
static void handle__click_callback(t_handle *sh, t_floatarg f){
    ...
    if(sh->h_dragon && click == 0){
        ...
        t_atom redo[1];
        int size = (int)((x->x_size+sh->h_dragx)/x->x_zoom);
        SETFLOAT(redo, size);
        ...
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -outline blue -width %d -tags %s\n",
            x->x_cv, x1, y1, x2, y2, KNOB_SELBDWIDTH*x->x_zoom, sh->h_outlinetag);
...
// only x for now
        newx = x2 + dx; // newy = y2 + dx;
        if(newx < x1 + MIN_SIZE*x->x_zoom)
            newx = x1 + MIN_SIZE*x->x_zoom;
        sys_vgui(".x%lx.c coords %s %d %d %d %d\n", x->x_cv, sh->h_outlinetag, x1, y1, newx, newx);
        sh->h_dragx = dx, sh->h_dragy = dy;
    }
}
