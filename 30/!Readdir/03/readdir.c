...
static void readdir_close(t_readdir *x)
{
  ...
  if (0 != closedir(x->x_dir)) {
    pd_error(x, "readdir: cannot close %s", x->x_dirname->s_name);
    return;
  }
  x->x_dir = NULL;
  x->x_dirname = sp_none;
...
static void readdir_open(t_readdir *x, t_symbol *dirname){
    ...
    if(x->x_dir){
        post("there's an open dr, close it");
        readdir_close(x);
    }
    if(!(x->x_dir = opendir(dirname->s_name))){
        pd_error(x, "readdir: cannot open %s", dirname->s_name);
        return;
    }
    x->x_dirname = dirname;
    post("we could open, so x->x_dirname = %s", x->x_dirname->s_name);
}
