...
void rand_seed(unsigned int *statep, unsigned int seed){ // borrowed from x_misc.c
    if(seed)
        *statep = (seed & 0x7fffffff);
    else{ // LATER consider using time elapsed from system startup,
        // (or login -- in linux we might call getutent)
        static unsigned int failsafe = 1489853723;
        static int shift = 0;
        static unsigned int lastticks = 0;
        // LATER rethink -- it might fail on faster machine than mine (but does it matter?)
...
int rand_int(unsigned int *statep, int range){ // borrowed from x_misc.c
