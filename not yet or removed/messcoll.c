...
static t_symbol * coll_fullfilename(t_messcoll *x, t_symbol * filename){
...
static int collelem_less(t_messcollelem *ep1, t_messcollelem *ep2, int ndx, int swap){
    int isless;
    if(swap){
        t_messcollelem *ep = ep1;
        ep1 = ep2;
        ep2 = ep;
    }
    if(ndx < 0){
...
static void collcommon_takeout(t_messcollcommon *cc, t_messcollelem *ep){
    if(ep->e_prev)
...
static void collcommon_remove(t_messcollcommon *cc, t_messcollelem *ep){
...
static void collcommon_putbefore(t_messcollcommon *cc, t_messcollelem *ep, t_messcollelem *next){
    if(next){
        ep->e_next = next;
...
static void collcommon_swaplinks(t_messcollcommon *cc, t_messcollelem *ep1, t_messcollelem *ep2){
    if(ep1 != ep2){
        t_messcollelem *prev1 = ep1->e_prev, *prev2 = ep2->e_prev;
        if(prev1 == ep2){
            ...
            if(prev2){
                ...
                t_messcollelem *next2 = ep2->e_next;
                collcommon_takeout(cc, ep1);
                collcommon_takeout(cc, ep2);
            ...
            t_messcollelem *next1 = ep1->e_next;
            collcommon_takeout(cc, ep1);
            collcommon_takeout(cc, ep2);
            collcommon_putafter(cc, ep1, prev2);
...
static void collcommon_swapkeys(t_messcollcommon *cc, t_messcollelem *ep1, t_messcollelem *ep2){
...
static void collcommon_changesymkey(t_messcollcommon *cc, t_messcollelem *ep, t_symbol *s){
    ep->e_symkey = s;
    collcommon_modified(cc, 0);
}

static void collcommon_renumber2(t_messcollcommon *cc, int startkey){
...
static void collcommon_renumber(t_messcollcommon *cc, int startkey){
    t_messcollelem *ep;
    for(ep = cc->c_first; ep; ep = ep->e_next)
        if(ep->e_hasnumkey)
...
static void collcommon_sort(t_messcollcommon *cc, int descending, int ndx){
    t_messcollelem *min = cc->c_first;
    t_messcollelem *ep;
...
static void collcommon_adddata(t_messcollcommon *cc, t_messcollelem *ep, int ac, t_atom *av){
    if(ac){
        t_atom *ap;
...
static t_messcollelem *collcommon_tonumkey(t_messcollcommon *cc, int numkey,
...
static int collcommon_frombinbuf(t_messcollcommon *cc, t_binbuf *bb){
...
static void collcommon_editorhook(t_pd *z, t_symbol *s, int ac, t_atom *av){
    (void)s;
    int nlines = collcommon_fromatoms((t_messcollcommon *)z, ac, av);
    if(nlines < 0)
...
/*static t_messcollcommon *coll_checkcommon(t_messcoll *x)
{ // unused
    if(x->x_name &&
	x->x_common != (t_messcollcommon *)pd_findbyclass(x->x_name,
						      messcollcommon_class))
    {
...
static void coll_unbind(t_messcoll *x){
...
static void coll_bind(t_messcoll *x, t_symbol *name){
    ...
    if(!cc){
        cc = (t_messcollcommon *)collcommon_new();
        cc->c_refs = 0;
        cc->c_increation = 0;
        //x->x_common = cc;
        //x->x_s = name;
...
static int coll_rebind(t_messcoll *x, t_symbol *name){
    t_messcollcommon *cc;
    if(name && name != &s_ && (cc = (t_messcollcommon *)pd_findbyclass(name, messcollcommon_class))){
        coll_unbind(x);
        x->x_common = cc;
        x->x_name = name;
        x->x_next = cc->c_refs;
...
static t_messcollelem *coll_firsttyped(t_messcoll *x, int ndx, t_atomtype type){
    t_messcollcommon *cc = x->x_common;
    t_messcollelem *ep;
    for(ep = cc->c_first; ep; ep = ep->e_next)
...
// methods -------------------------------------------------------------------------------------
static void coll_wclose(t_messcoll *x){ // if edited, closing window asks and replace the contents
...
static void coll_anything(t_messcoll *x, t_symbol *s, int ac, t_atom *av){
    (void)av;
    if(ac == 0)
        coll_symbol(x, s);
}
...
static void coll_nstore(t_messcoll *x, t_symbol *s, int ac, t_atom *av){
    if(ac >= 3){
		t_messcollcommon *cc = x->x_common;
		t_messcollelem *ep;
		int numkey;
...
static void coll_insert2(t_messcoll *x, t_symbol *s, int ac, t_atom *av){
    t_messcollcommon  *cc = x->x_common;
    t_messcollelem *epnew = NULL;
    t_messcollelem *ep;
    int numkey;
    if(ac >= 2 && av->a_type == A_FLOAT){
...
static void coll_insert(t_messcoll *x, t_symbol *s, int ac, t_atom *av){
    t_messcollcommon  *cc = x->x_common;
    t_messcollelem *epnew = NULL;
    t_messcollelem *ep;
    int numkey;
...
static void coll_remove(t_messcoll *x, t_symbol *s, int ac, t_atom *av){
    if(ac){
		t_messcollelem *ep;
...
static void coll_assoc(t_messcoll *x, t_symbol *s, t_floatarg f){
    int numkey;
    if(coll_checkint((t_pd *)x, f, &numkey, gensym("assoc"))){
        t_messcollcommon *cc = x->x_common;
        t_messcollelem *ep1, *ep2;
...
static void coll_deassoc(t_messcoll *x, t_symbol *s, t_floatarg f){
    (void)s;
    int numkey;
...
static void coll_subsym(t_messcoll *x, t_symbol *s1, t_symbol *s2){
    t_messcollelem *ep;
...
static void coll_renumber2(t_messcoll *x, t_floatarg f){
    int startkey;
...
static void coll_renumber(t_messcoll *x, t_floatarg f){
    int startkey;
    if(coll_checkint((t_pd *)x, f, &startkey, gensym("renumber"))){
...
static void coll_merge(t_messcoll *x, t_symbol *s, int ac, t_atom *av){
    if(ac >= 2){
        t_messcollcommon *cc = x->x_common;
        t_messcollelem *ep;
        if(av->a_type == A_FLOAT){
...
static void coll_sub(t_messcoll *x, t_symbol *s, int ac, t_atom *av){
    if(ac){
        t_messcollelem *ep;
        if((ep = coll_findkey(x, av, s))){
            t_messcollcommon *cc = x->x_common;
...
static void coll_sort(t_messcoll *x, t_floatarg f1, t_floatarg f2){
    int dir, ndx;
...
static void coll_swap(t_messcoll *x, t_symbol *s, int ac, t_atom *av){
    if(ac == 2){
		t_messcollelem *ep1, *ep2;
...
static void coll_nth(t_messcoll *x, t_symbol *s, int ac, t_atom *av){
...
static void coll_length(t_messcoll *x){
    t_messcollcommon *cc = x->x_common;
...
static void coll_min(t_messcoll *x, t_floatarg f){
    int ndx;
    if(coll_checkint((t_pd *)x, f, &ndx, gensym("min"))){
        t_messcollelem *found;
        if(ndx > 0)
            ndx--;
...
static void coll_max(t_messcoll *x, t_floatarg f){
    int ndx;
...
static void coll_refer(t_messcoll *x, t_symbol *s){
    coll_rebind(x, s);
}

static void coll_flags(t_messcoll *x, t_float f1){
    int i1;
...
static void coll_embed(t_messcoll *x, t_float f){
    int embed = f == 0 ? 0 : 1;
    coll_flags(x, (t_float) embed);
}
...
static void coll_readagain(t_messcoll *x){
    t_messcollcommon *cc = x->x_common;
    if(cc->c_filename){
        if(x->x_threaded){
            x->unsafe = 2;
...
static void coll_writeagain(t_messcoll *x){
    t_messcollcommon *cc = x->x_common;
    if(cc->c_filename) {
        if(x->x_threaded == 1) {
            x->unsafe = 11;
...
static void coll_filetype(t_messcoll *x, t_symbol *s, int argc, t_atom * argv){
    (void)s;
    int ext_set = 0;
    if(argc > 0){
...
/* #ifdef COLL_DEBUG
static void collelem_post(t_messcollelem *ep)
{
		if(ep->e_hasnumkey && ep->e_symkey)
...
static void collcommon_post(t_messcollcommon *cc)
{
    t_messcollelem *ep;
    for(ep = cc->c_first; ep; ep = ep->e_next) collelem_post(ep);
}

static void coll_debug(t_messcoll *x, t_floatarg f)
{
		t_messcollcommon *cc = coll_checkcommon(x);
		if(cc)
		{
			t_messcoll *x1 = cc->c_refs;
			t_messcollelem *ep, *last;
			int i = 0;
...
static void coll_separate(t_messcoll *x, t_floatarg f){
	int indx;
	t_messcollcommon *cc = x->x_common;
...
static void *coll_new(t_symbol *s, int argc, t_atom *argv){
    ...
    x->x_filehandle = elsefile_new((t_pd *)x, coll_embedhook, 0, 0, 0);
    int no_search = 0;
    int embed = COLLEMBED;
...
void messcoll_setup(void){
    ...
    class_addbang(messcoll_class, coll_next);
    class_addfloat(messcoll_class, coll_float);
    class_addsymbol(messcoll_class, coll_symbol);
    class_addlist(messcoll_class, coll_list);
    class_addanything(messcoll_class, coll_anything);
    class_addmethod(messcoll_class, (t_method)coll_store, gensym("store"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_nstore, gensym("nstore"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_insert, gensym("insert"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_insert2, gensym("insert2"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_remove, gensym("remove"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_delete, gensym("delete"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_assoc, gensym("assoc"), A_SYMBOL, A_FLOAT, 0);
    class_addmethod(messcoll_class, (t_method)coll_deassoc, gensym("deassoc"), A_SYMBOL, A_FLOAT, 0);
    class_addmethod(messcoll_class, (t_method)coll_subsym, gensym("subsym"), A_SYMBOL, A_SYMBOL, 0);
    class_addmethod(messcoll_class, (t_method)coll_renumber, gensym("renumber"), A_DEFFLOAT, 0);
    class_addmethod(messcoll_class, (t_method)coll_renumber2, gensym("renumber2"), A_DEFFLOAT, 0);
    class_addmethod(messcoll_class, (t_method)coll_merge, gensym("merge"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_sub, gensym("sub"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_sub, gensym("nsub"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_clear, gensym("clear"), 0);
    class_addmethod(messcoll_class, (t_method)coll_sort, gensym("sort"), A_FLOAT, A_DEFFLOAT, 0);
    class_addmethod(messcoll_class, (t_method)coll_swap, gensym("swap"), A_GIMME, 0);
    ...
    class_addmethod(messcoll_class, (t_method)coll_nth, gensym("nth"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_length, gensym("length"), 0);
    class_addmethod(messcoll_class, (t_method)coll_min, gensym("min"), A_DEFFLOAT, 0);
    class_addmethod(messcoll_class, (t_method)coll_max, gensym("max"), A_DEFFLOAT, 0);
    class_addmethod(messcoll_class, (t_method)coll_refer, gensym("refer"), A_SYMBOL, 0);
    class_addmethod(messcoll_class, (t_method)coll_embed, gensym("embed"), A_FLOAT,0);
    ...
    class_addmethod(messcoll_class, (t_method)coll_readagain, gensym("readagain"), 0);
    class_addmethod(messcoll_class, (t_method)coll_writeagain, gensym("writeagain"), 0);
    class_addmethod(messcoll_class, (t_method)coll_filetype, gensym("filetype"), A_GIMME, 0);
    class_addmethod(messcoll_class, (t_method)coll_dump, gensym("dump"), 0);
    class_addmethod(messcoll_class, (t_method)coll_open, gensym("open"), 0);
    class_addmethod(messcoll_class, (t_method)coll_wclose, gensym("wclose"), 0);
    class_addmethod(messcoll_class, (t_method)coll_click, gensym("click"),
        A_FLOAT, A_FLOAT, A_FLOAT, A_FLOAT, A_FLOAT, 0);
    class_addmethod(messcoll_class, (t_method)coll_separate, gensym("separate"), A_FLOAT, 0);
    class_addmethod(messcoll_class, (t_method)coll_is_opened, gensym("_is_opened"), A_FLOAT , A_FLOAT, 0);
/* #ifdef COLL_DEBUG
    class_addmethod(messcoll_class, (t_method)coll_debug,
		    gensym("debug"), A_DEFFLOAT, 0);
#endif */
    elsefile_setup(messcoll_class, 1);
    messcollcommon_class = class_new(gensym("coll"), 0, 0, sizeof(t_messcollcommon), CLASS_PD, 0);
