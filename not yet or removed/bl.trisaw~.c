...
static t_class *bltrisaw_class;

double bltrisaw_wrap_phase(double phase){
    while(phase >= 1)
        phase -= 1.;
    while(phase < 0)
        phase += 1.;
...
static t_int *bltrisaw_perform(t_int *w){
    t_bltrisaw *x = (t_bltrisaw *)(w[1]);
        ...
        for(int i = 0, n = x->x_n; i < n; i++){
            ...
            if(x->x_midi && hz == 0)
               out[j*n + i] = 0.;
                ...
                double p = phase[j] * 2.0;

                if(p > 1.0)
                    p = 1.0 - p;

                out[j*n + i] = p + elliptic_blep_get(&blep[j]);
                ...
                if(phase[j] >= 1 || phase[j] < 0) {
                    t_float phase_step = bltrisaw_wrap_phase(x->x_phase[j] - last_phase);
                    t_float amp_step = 1.0f;
                    phase[j] = bltrisaw_wrap_phase(phase[j]);
                    t_float samples_in_past = phase[j] / phase_step;
                    elliptic_blep_add_in_past(&blep[j], amp_step, 1, samples_in_past);
                }
                else if (phase[j] >= 0.5 && phase[j] < 0.5 + step) {
                    t_float phase_step = bltrisaw_wrap_phase(x->x_phase[j] - last_phase);
                    t_float amp_step = -1.0f;
                    t_float samples_in_past = (phase[j] - 0.5) / phase_step;
                    elliptic_blep_add_in_past(&blep[j], amp_step, 1, samples_in_past);
                }
            }
        }
...
static void bltrisaw_dsp(t_bltrisaw *x, t_signal **sp){
    ...
    || (x->x_ch3 > 1 && x->x_ch3 != x->x_nchans)){
        ...
        if(x->x_verbose)
            pd_error(x, "[bl.trisaw~]: channel sizes mismatch");
        return;
    }
    dsp_add(bltrisaw_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec);
}

static void bltrisaw_midi(t_bltrisaw *x, t_floatarg f){
    x->x_midi = (int)(f != 0);
}

static void bltrisaw_set(t_bltrisaw *x, t_symbol *s, int ac, t_atom *av){
    (void)s;
    if(ac != 2)
        return;
...
static void bltrisaw_list(t_bltrisaw *x, t_symbol *s, int ac, t_atom * av){
    (void)s;
    if(ac == 0)
        return;
    if(x->x_list_size != ac){
...
static void bltrisaw_soft(t_bltrisaw *x, t_floatarg f){
    x->x_soft = (int)(f != 0);
}

static void bltrisaw_verbose(t_bltrisaw *x, t_floatarg f){
    x->x_verbose = (int)(f != 0);
}

static void bltrisaw_free(t_bltrisaw *x) {
...
static void *bltrisaw_new(t_symbol *s, int ac, t_atom *av){
    t_bltrisaw *x = (t_bltrisaw *)pd_new(bltrisaw_class);
    (void)s;
    x->x_midi = x->x_soft = 0;
    ...
    return(x);
errstate:
    post("[bltrisaw~]: improper args");
    return(NULL);
}

void setup_bl0x2etrisaw_tilde(void){
    bltrisaw_class = class_new(gensym("bl.trisaw~"), (t_newmethod)bltrisaw_new, (t_method)bltrisaw_free,
        sizeof(t_bltrisaw), CLASS_MULTICHANNEL, A_GIMME, 0);
    class_addmethod(bltrisaw_class, nullfn, gensym("signal"), 0);
    class_addmethod(bltrisaw_class, (t_method)bltrisaw_dsp, gensym("dsp"), A_CANT, 0);
    class_addlist(bltrisaw_class, bltrisaw_list);
    class_addmethod(bltrisaw_class, (t_method)bltrisaw_verbose, gensym("verbose"), A_FLOAT, 0);
    class_addmethod(bltrisaw_class, (t_method)bltrisaw_soft, gensym("soft"), A_FLOAT, 0);
    class_addmethod(bltrisaw_class, (t_method)bltrisaw_midi, gensym("midi"), A_FLOAT, 0);
    class_addmethod(bltrisaw_class, (t_method)bltrisaw_set, gensym("set"), A_GIMME, 0);
}
