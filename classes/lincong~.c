...
static void lincong_k(t_lincong *x, t_float f)
{
    //    x->x_a = f;
    //    x->x_c = f;
    //    x->x_m = f;
...
/*    if (argc > 2)
        {
        pd_error(x, "lincong~: list size needs to be = 2");
        }
    else{
...
void lincong_tilde_setup(void)
{
    ...
    class_addmethod(lincong_class, (t_method)lincong_k, gensym("k"), A_DEFFLOAT, 0);
}
