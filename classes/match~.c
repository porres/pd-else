...
t_int *match_perform(t_int *w)
{
    ...
    for(i = 0; i < 1; i++){ // copy input vector
        invec = (t_float *) w[2 + i];
        for(j = 0; j < n; j++)
            ins[i][j] = invec[j];
    }
...
static void *match_new(t_symbol *s, int argc, t_atom *argv){
    int i;
...
{
    match_class = class_new(gensym("match~"), (t_newmethod)match_new, 0,
            sizeof(t_match), CLASS_DEFAULT, A_GIMME, 0);
