...
void noisy_tilde_setup(void)
{
    noisy_class = class_new(gensym("noisy~"),
        ...
        sizeof(t_noisy), CLASS_DEFAULT, A_DEFFLOAT, 0);
    CLASS_MAINSIGNALIN(noisy_class, t_noisy, x_freq);
    class_addmethod(noisy_class, (t_method)noisy_class, gensym("dsp"), A_CANT, 0);
}
