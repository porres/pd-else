...
void mtx_dsp(t_mtx *x, t_signal **sp){
    ...
    if(x->x_vtemp){
        ...
        if(vec){
            ...
            for(i = 1; i <= x->x_ch * 2; ++i)
                vec[i+3] = (t_int)sp[i]->s_vec; // vec[i+4]
            dsp_addv(mtx_perform, (x->x_ch * 2 + 4), vec); // x->x_ch * 2 + 4
            freebytes(vec, (x->x_ch * 2 + 2) * sizeof(t_int)); // change?? to "+ 3"?s
        }
        else
...
void mtx_tilde_setup(void){
    mtx_class = class_new(gensym("mtx~"), (t_newmethod)mtx_new, NULL,
        sizeof(t_mtx), CLASS_DEFAULT, A_DEFFLOAT, 0);
