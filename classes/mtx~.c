...
{ // CHECKED same output as 'dump' -> [mtx~] -> [print]
...
{
    mtx_class = class_new(gensym("mtx~"),
			     (t_newmethod)mtx_new,
			     (t_method)mtx_free,
			     sizeof(t_mtx), 0, A_GIMME, 0);
...
//    class_addmethod(mtx_class, (t_method)mtx_connect, gensym("connect"), A_GIMME, 0);
//    class_addmethod(mtx_class, (t_method)mtx_connect, gensym("disconnect"), A_GIMME, 0);
...
//    class_addmethod(mtx_class, (t_method)mtx_dumptarget, gensym("dumptarget"), 0);
