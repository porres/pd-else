...
static t_int *sawtooth_perform_sig(t_int *w)
{
    t_sawtooth *x = (t_sawtooth *)(w[1]);
    int nblock = (t_int)(w[2]);
    t_float *in1 = (t_float *)(w[3]); // freq
...
static void sawtooth_dsp(t_sawtooth *x, t_signal **sp){
    ...
    x->x_sr = sp[0]->s_sr;
    if (x->x_hasfeeders){
        dsp_add(sawtooth_perform_sig, 6, x, sp[0]->s_n,
            sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec);
        }
    else{
        dsp_add(sawtooth_perform, 6, x, sp[0]->s_n, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec);
    }
}
...
static void *sawtooth_new(t_symbol *s, int ac, t_atom *av){
...
void sawtooth_tilde_setup(void){
    sawtooth_class = class_new(gensym("sawtooth~"),
        ...
        sizeof(t_sawtooth), CLASS_DEFAULT, A_GIMME, 0);
