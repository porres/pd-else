...
typedef struct _toany_char_code{
...
typedef struct _toany{
  t_object       x_obj;
  t_toany_char_code x_char_code;    // byte buffer: {b_buf~x_text,b_len~?,b_alloc~x_size}
  t_float        x_eos;     // eos value
  t_binbuf      *x_binbuf;
  t_inlet       *x_eos_in;
  t_outlet      *x_outlet;
}t_toany;
...
toany_get_atoms(void *x, t_toany_char_code *dst, t_toany_atoms *src, t_float x_eos){
    ...
    if(dst->b_alloc <= (size_t)(argc + 1)) // re-allocate?
        toany_char_code_realloc(dst, argc + 1 + 256);
  ...
  if(x->x_eos >= 0){
    for(i = i0; i < argc; i++){
        if(((int)atom_getfloatarg(i, argc, argv)) == ((int)x->x_eos)){
            toany_output(x, i - i0, argv + i0);
            i0 = i + 1;
        }
    }
  }
  if(i0 < argc)
    toany_output(x, argc - i0, argv + i0);
}
