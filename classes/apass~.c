...
#define apass_DEFGAIN 0. //default gain
...
static void apass_dsp(t_apass *x, t_signal **sp)
{
    ...
    dsp_add(apass_perform, 6, x, sp[0]->s_n,
            sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec,
            sp[3]->s_vec);
}

static void apass_list(t_apass *x, t_symbol *s, int argc, t_atom * argv){
    ...
    while(argc){
        if(argv -> a_type == A_FLOAT){
            ...
            switch(argnum){
                case 0:
                    //maxdel
                    x->x_maxdel = curf > 0 ? curf : apass_DELAY;
                    apass_sz(x);
                    break;
                case 1:
                    //initdel
...
static void *apass_new(t_symbol *s, int argc, t_atom * argv){
    ...
    //defaults
    t_float maxdel = apass_DELAY;
    t_float initdel = apass_MINMS;
    t_float gain = apass_DEFGAIN;
    x->x_sr = sys_getsr();
...
{
    apass_class = class_new(gensym("apass~"),
                              (t_newmethod)apass_new,
                              (t_method)apass_free,
                              sizeof(t_apass), 0,
                              A_GIMME, 0);
    ...
    class_addlist(apass_class, (t_method)apass_list);
}
