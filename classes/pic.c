...
// TODO callback from gui sys_vgui("image_size logo");
     }     
     else{
...
void image_open(t_image* x, t_symbol *s){
    x->x_fname = s;
...
static t_symbol *get_filename(t_int argc, t_atom *argv){
    t_symbol *fname = atom_getsymbolarg(0, argc, argv);
    int i;
...
static void *image_new(t_symbol *s, t_int argc, t_atom *argv){
    ...
    post("version %d", PD_MINOR_VERSION);
    return (x);
}

void pic_setup(void){
    image_class = class_new(gensym("pic"), (t_newmethod)image_new, 0, sizeof(t_image),0, A_GIMME,0);
    class_addmethod(image_class, (t_method)image_open, gensym("open"), A_DEFSYMBOL, 0);
    image_setwidget();
